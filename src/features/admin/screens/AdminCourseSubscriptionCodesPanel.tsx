...
export function AdminCourseSubscriptionCodesPanel({
  courseId,
  courseSlug,
  packageId,
  packageTitle,
  ...
  const [codes, setCodes] = useState<SubscriptionCodeRow[]>([]);
  ...
  const deleteCodesFromDb = async (ids: string[]) => {
    ...
    const res = await supabase.from("subscription_codes").delete().in("id", list);
    if (res.error) {
  ...
  const copyOne = async (row: SubscriptionCodeRow) => {
    setCopyingId(row.id);
    setError(null);
    try {
  ...
  const titleLabel = useMemo(() => {
    const t = String(packageTitle ?? "").trim();
    if (t) return `أكواد اشتراك الكورس • ${t}`;
    return "أكواد اشتراك الكورس";
  }, [packageTitle]);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const res = await supabase
        ...
        .select("id,code,course_id,package_id,duration_days,max_redemptions,active,note,created_at")
      ...
      const list = (res.data as SubscriptionCodeRow[]) ?? [];
      setCodes(list);
      ...
      const redRes = await supabase.from("code_redemptions").select("code_id").in("code_id", ids);
  ...
  }, [courseId, packageId, reloadKey]);

  const generate = async () => {
    const c = toIntOrNull(count);
    const d = toIntOrNull(durationDays);
    const max = toIntOrNull(maxRedemptions);
  ...
  const deleteCode = async (id: string) => {
    ...
    const res = await supabase.from("subscription_codes").delete().eq("id", id);
    if (res.error) {
      setError(res.error.message);
      setDeletingId(null);
      return;
  ...
  const toggleActive = async (id: string, active: boolean) => {
    ...
    const res = await supabase.from("subscription_codes").update({ active }).eq("id", id);
  ...
  if (!courseId) {
    return (
      <AdminCard>
        <div className="font-heading text-xl tracking-[0.12em] text-slate-900">أكواد اشتراك الكورس</div>
  ...
  if (!packageId) {
    return (
      <AdminCard>
        <div className="font-heading text-xl tracking-[0.12em] text-slate-900">أكواد اشتراك الكورس</div>
        <div className="mt-2 text-sm text-slate-600">اختر باقة أولاً.</div>
      </AdminCard>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <AdminCard>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="font-heading text-xl tracking-[0.12em] text-slate-900">{titleLabel}</div>
            <div className="mt-2 text-sm text-slate-600">توليد أكواد اشتراك للكورس مرتبطة بالباقة.</div>
          </div>
      ...
      <AdminCard>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-xs text-slate-500">{codes.length} كود</div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              disabled={saving || copyingAll || codes.length === 0}
